...
#
# Pré-requis : build local des bins (target/debug/devit, devit-mcp, mcp-server)
...
MCP_SERVER_BIN="${MCP_SERVER_BIN:-target/debug/mcp-server}"
SERVER_CMD_BASE="$MCP_SERVER_BIN --working-dir $(pwd)"
...
# Detect supported flags on mcp-server (legacy flags may be absent)
MCP_SERVER_HELP="$("$MCP_SERVER_BIN" --help 2>&1 || true)"
FLAGS_SECRETS=""
if echo "$MCP_SERVER_HELP" | grep -q -- "--env-allow"; then FLAGS_SECRETS+=" --env-allow PATH,HOME"; fi
if echo "$MCP_SERVER_HELP" | grep -q -- "--secrets-scan"; then FLAGS_SECRETS+=" --secrets-scan"; fi
if echo "$MCP_SERVER_HELP" | grep -q -- "--redact-placeholder"; then FLAGS_SECRETS+=" --redact-placeholder '***REDACTED***'"; fi
...
RED_OUT="$("$MCP_BIN" --cmd "$SERVER_CMD_BASE$FLAGS_SECRETS" --call devit.tool_call --json "$PAY_RED" || true)"
...
if command -v bwrap >/dev/null 2>&1; then
  FLAGS_NET=""
  if echo "$MCP_SERVER_HELP" | grep -q -- "--sandbox"; then FLAGS_NET+=" --sandbox bwrap"; fi
  if echo "$MCP_SERVER_HELP" | grep -q -- "--net"; then FLAGS_NET+=" --net off"; fi
  OUT=$("$MCP_BIN" --cmd "$SERVER_CMD_BASE$FLAGS_NET" \
