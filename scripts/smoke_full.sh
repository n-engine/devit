...
echo "[1/6] Build binaries (devit, devit-mcp, mcp-server)…" >&2
...
if command -v bwrap >/dev/null 2>&1; then
  "$BIN_MCP" --cmd "'$BIN_MCP_SERVER' --working-dir '$ROOT_DIR'" --call devit.tool_list --json '{}' | rg -n 'tool.result|tool.error' || true
else
