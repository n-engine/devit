...
if have target/debug/devit-mcp && have target/debug/mcp-server; then
  rt 10 bash -lc "target/debug/devit-mcp --cmd \"target/debug/mcp-server --working-dir $(pwd)\" --handshake-only >'$HANDSHAKE_JSON'" || true
  rt 10 bash -lc "target/debug/devit-mcp --cmd \"target/debug/mcp-server --working-dir $(pwd)\" --call devit.tool_list --json '{}' >'$TOOLS_JSON'" || true
fi
