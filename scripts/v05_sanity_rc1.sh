...
#
# Prérequis: bins buildés dans target/debug (devit, devit-mcp, mcp-server, devit-tui)
...
MCP_SERVER="${MCP_SERVER_BIN:-target/debug/mcp-server}"
...
for b in "$DEVIT" "$MCP" "$MCP_SERVER" "$TUI"; do
...
# Vérifie policy (shell_exec on_request attendu par défaut)
SERVER_CMD="$MCP_SERVER --working-dir $(pwd)"
POLICY_JSON="$("$MCP" --cmd "$SERVER_CMD" --call server.policy --json '{}' 2>/dev/null | sed -n '2p' || true)"
