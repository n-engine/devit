...
impl McpClient {
    pub fn spawn_cmd(cmd: &str, per_msg_timeout: Duration) -> Result<Self> {
        // Use platform shell wrapper (bash -c | cmd /C)
        // FIXME(windows): The provided `cmd` string may contain Bash-specific syntax
        // (pipes/quotes). If so, Git Bash must be available on PATH.
